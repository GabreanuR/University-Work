...
--Exercitiu Prezentare

CREATE TABLE COPIE_TRASEE_STATIE AS
SELECT *
FROM TRASEE_STATIE;

ALTER TABLE COPIE_TRASEE_STATIE
    ADD PRIMARY KEY (id_statie, id_linie);

ALTER TABLE COPIE_TRASEE_STATIE
    ADD FOREIGN KEY (id_statie) REFERENCES STATII_TRANSPORT (id_statie);

ALTER TABLE COPIE_TRASEE_STATIE
    ADD FOREIGN KEY (id_linie) REFERENCES LINII_TRANSPORT_COMUN (id_linie);

SELECT UNIQUE V.ID_VEHICUL, V.TIP, V.NUMAR_INMATRICULARE
FROM VEHICULE V
         JOIN TRECERI_VEHICULE TV ON V.ID_VEHICUL = TV.ID_VEHICUL
         JOIN CAMERE C ON TV.ID_CAMERA = C.ID_CAMERA
         JOIN STRAZI STR ON C.ID_STRADA = STR.ID_STRADA
WHERE STR.ID_STRADA = (SELECT STR2.ID_STRADA
                       FROM STRAZI STR2
                                JOIN STATII_TRANSPORT STS ON STR2.ID_STRADA = STS.ID_STRADA
                       GROUP BY STR2.ID_STRADA
                       HAVING COUNT(STS.ID_STATIE) = (SELECT MAX(COUNT(STS2.ID_STATIE))
                                                      FROM STATII_TRANSPORT STS2
                                                               JOIN STRAZI STR3 ON STS2.ID_STRADA = STR3.ID_STRADA
                                                      GROUP BY STR3.ID_STRADA))
